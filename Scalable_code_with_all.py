...
input_data=pd.merge(input_data, dist_h, how='inner', left_on= 'PROJECT_ID', right_on = 'project_id')


input_data=pd.merge(input_data, coast, how='inner', left_on= 'PROJECT_ID', right_on = 'project_id')

input_data['rad_dist'] = Min_Radial_distcsv['Rad_dist']
...
model_results = pd.DataFrame(columns = ['LOCATIONID', 'location_name', 'row_count', 'project_count', 'r2_test', 'rmse', 'mean_cost', 'percent_error', 'r_squared', 'f_statistic',
                                                ...
                                                'co_time','co_dis_s','co_dis_h', 'co_dis_c','se_size','se_storey',
                                                'se_time','se_dis_s', 'se_dis_h', 'se_dis_c', 'p_size','p_storey',
                                                'p_time','p_dis_s','p_dis_h', 'p_dis_c', 'condition_no'])
for i in input_data.LOCATIONID.unique():
    ...
    ##Assessing and essential libraries installation
        ...
        X = p1[['SALEABLE_FLAT_SIZE','STOREY', 'diff', 'DISTANCE_KM', 'dist_x', 'dist_y' ]]
        ...
        co_time = model.params.iloc[2]
        co_dis_s = model.params.iloc[3]
        co_dis_h = model.params.iloc[4]
        co_dis_c = model.params.iloc[5]
        se_size = model.bse.iloc[0]
        se_storey = model.bse.iloc[1] 
        se_time = model.bse.iloc[2]
        se_dis_s = model.bse.iloc[3]
        se_dis_h = model.bse.iloc[4]
        se_dis_c = model.bse.iloc[5]
        p_size = model.pvalues.iloc[0]
        p_storey = model.pvalues.iloc[1] 
        p_time = model.pvalues.iloc[2]
        p_dis_s = model.pvalues.iloc[3]
        p_dis_h = model.pvalues.iloc[4]
        p_dis_c = model.pvalues.iloc[5]
        ...
        single_results = pd.DataFrame([[i, location_name, row_count, project_count, r2_test, rmse, mean_cost, percent_error, r_squared, f_statistic,
                                                ...
                                                co_time, co_dis_s, co_dis_h, co_dis_c, se_size, se_storey,
                                                se_time, se_dis_s, se_dis_h, se_dis_c, p_size, p_storey,
                                                p_time, p_dis_s, p_dis_s, p_dis_s, condition_no]], columns = ['LOCATIONID', 'location_name', 'row_count', 'project_count', 'r2_test', 'rmse', 'mean_cost', 'percent_error', 'r_squared', 'f_statistic',
                                                'p_value_f', 'co_size','co_storey',
                                                'co_time','co_dis_s','co_dis_h', 'co_dis_c','se_size','se_storey',
                                                'se_time','se_dis_s', 'se_dis_h', 'se_dis_c', 'p_size','p_storey',
                                                'p_time','p_dis_s','p_dis_h', 'p_dis_c', 'condition_no'])
